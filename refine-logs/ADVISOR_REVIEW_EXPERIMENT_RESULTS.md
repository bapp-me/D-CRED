...
### M4: Proxy Economic Utility And Feature Acquisition Pilot - SUPERSEDED

Output directory: `outputs/economic_feature_acquisition_pilot/`

**Superseded note (2026-05-20):** This proxy-only pilot is historical. The downloaded accepted-loan `loan.csv` does contain observed repayment cash-flow fields, and the authoritative dean-response evidence is now the clean rerun in `outputs/dean_cashflow_full/` plus `refine-logs/DEAN_CASHFLOW_EXPERIMENT_RESULTS.md`. Do not cite the proxy-only pilot as final evidence for the dean's cash-flow critique.
